...
That is the whole demo — **Phase 1, text path.** Phase 2 leads with the
multimodal agent: drop a photo of a building into the CHAT panel, type
one sentence, and the agent reads the image, emits a plan, and calls
IfcWall / IfcSlab / IfcDoor against the same dispatch table. Multi-turn
refinements ("add a garage") accumulate in conversation history. The
geometry still renders in WebAssembly; the IFC export is the same
standard file. The agent is the only remote call.

60 cached Gemma 4 LoRA outputs ship with the page; sub-100ms F1 fuzzy
match is the default judge experience for the text path. The geometry
kernel and IFC4 emitter run in-browser via WebAssembly. Live LoRA
inference is opt-in: `src/serve/serve_lora.py` (FastAPI + Unsloth or
vLLM) serves a 4090-resident adapter on port 8088 when
`VITE_GEMMA_AGENT_URL` is set; the same endpoint handles vision requests
...
- **Multimodal agent path (shipped D1/D2/D3).** Paste or drop an image
  into the CHAT panel. `chat-panel.ts` encodes it as a data URL via
  `FileReader`, passes it through `agent-harness.ts` as `userImage`. The
  in-browser path wraps it with `RawImage.fromURL()` for the
  transformers.js pipeline; the remote path emits an OpenAI-compat
  `image_url` content block to `serve_lora.py`'s
  `/v1/chat/completions` endpoint (D2). The agent's tool calls dispatch
  into the IFC verb table — IfcWall, IfcSlab, IfcDoor, IfcCurtainWall —
  so one image drop + one sentence produces a loadable BIM file.
  Multi-turn: the image is consumed on the first turn; follow-up text
  refinements ("add a garage on the south side") accumulate in
  conversation history without re-upload. Viewport snapshots are
  generated by `bun run snapshot` (D3) and land in
  `web/public/snapshots/` for use as demo-script source images.
...
### Multimodal agent path

The CHAT dock tab (`web/src/chat-panel.ts`) accepts images via paste,
drag-drop, and a file picker. On send, the data URL travels through
`runAgentTurn({ ..., userImage })` in `agent-harness.ts`. Two code paths:

- **In-browser** (transformers.js local inference): `RawImage.fromURL(dataUrl)`
  wraps the attachment; the image + text tuple feeds
  `Gemma4ForConditionalGeneration` directly.
- **Remote** (`VITE_GEMMA_AGENT_URL` set): the `userImage` becomes an
  OpenAI-compat `[{type:"image_url", image_url:{url}}, {type:"text",
  text:prompt}]` content array in the POST body, routed to
  `serve_lora.py`'s `/v1/chat/completions` endpoint (vision-capable,
  D2).

The system prompt instructs the model to emit `<tool_call>` envelopes
(C1 class). The harness extracts `verb` + `args` from each envelope and
dispatches to `invokeCommand`, which validates against
`spatial-api.yaml` and fires the handler. Every IFC geometry verb
(`IfcWall`, `IfcSlab`, `IfcDoor`, `IfcCurtainWall`, …) is a registered
handler. Multi-turn history accumulates as plain text; the image is
consumed on the first turn.

### Skill canvas (P5a / P5b)

The NODES dock tab hosts a hand-rolled graph canvas (`skill-canvas.ts`):
zero graph-library dependency, absolutely-positioned node divs, SVG cubic
bezier edges, localStorage persistence. Users drag IFC verbs from a
palette onto the canvas, wire them with port clicks, set args inline, and
hit **Run** (topo-sorted dispatch) or **Compile to skill** (serialize to
`skill-store.ts`).

P5b closes the agent ↔ canvas loop. When the user types `"run <skill
name>"` in CHAT, `chat-panel.ts` checks `listSavedSkills()` for a prefix
match, emits a `skill:animate` CustomEvent with the matched steps, and
returns immediately. `workbench.ts` handles the event: switches the dock
to NODES, activates the canvas view, and calls
`SkillCanvas.runWithAnimation(steps)`, which builds a fresh linear graph
and highlights each node in topo order at 80ms cadence as its verb
dispatches. The agent's output is auditable — every dispatch fires in
sequence on screen.
...
- **Demo video**: 3:30 multimodal-first cut (Phase 2) is the submission
  video; 3:00 text-path fallback (Phase 1) ships as backup. Both scripts
  in `submission/demo-script.md`. YouTube URL filled at submission time.
...
The pipeline ends with a real IFC4 file the user downloads. Not a
screenshot. Not a JSON blob. A file that opens in the same software the
architect across town is paying $3K/year to use. That's the equity claim
— not "we made architecture more impressive," but "we made architecture's
output format accessible from a free webpage."

And the Phase 2 path sharpens it: drop a photo of a building you like,
type a sentence describing what you need, and the agent builds it. The
model isn't generating pixels. It's generating a building. The image is
the brief.
