...
`B:/M/gemma-architect/` is the **shared working tree** — Archie develops in it and Jun's vite dev server at `localhost:5175` serves from it. Whatever branch is checked out there is what Jun sees.

## Rules

1. **Never `git checkout <branch>` in `B:/M/gemma-architect/`** unless you're Archie and are deliberately switching what Jun sees. `bash`'s `cd` resets between invocations; `git checkout` writes to disk and persists across them.
...
4. **Never run a parallel vite from a worktree on the same port (5175)**. Either use a different port (and confirm with Jun before pointing him at it — see gate-discipline rule about port redirects), or don't run vite at all from the worktree.

## When the shared tree's branch DOES need to change

Only when verifying a different PR end-to-end on Jun's URL. Coordinate with Jun first:

1. Confirm Jun is ready to switch his view (mail him; wait for ack).
2. Switch the shared tree to the target branch.
3. Verify Jun sees the new state.
4. Switch back to the prior branch when done.

For PR review without Jun-side verification, use a worktree + a separate dev server on a different port; never touch the shared tree.
...
The third command returns the process serving Jun's URL; its working directory is in the CommandLine. Cross-check that working dir's HEAD before assuming what Jun sees.
