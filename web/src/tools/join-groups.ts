 ...
 * Subtracting this from the CSG union result removes the star/stair artifact at any angle.
 */
function _wallMiterCutter(wallA: THREE.Mesh, wallB: THREE.Mesh): Brush | null {
  ...
  // Find closest endpoint pair — identifies the junction end of each wall.
  ...
  // The box is ROTATED so its local X axis aligns with miterNormal — an
  // axis-aligned box would straddle the plane and incorrectly remove wall bodies.
  const LARGE = 500;
...
function _rebuildGroupDisplay(scene: THREE.Scene, groupId: string, primaryMat: THREE.Material): void {
  ...
  // Compute miter cutters for all wall-to-wall L-junctions.
