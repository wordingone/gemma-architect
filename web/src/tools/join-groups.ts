...
/**
 * Build a CSG Brush for a wall mesh. Extends the wall by its own thickness (t) at both
 * ends so the junction region is fully covered. At a two-wall junction each wall extends
 * t/2 past the shared endpoint; the extensions are mutually covered by the adjacent wall's
 * body, so the union fills the corner cleanly at any angle with no protrusions.
 */
